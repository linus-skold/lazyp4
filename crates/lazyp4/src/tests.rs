...
fn app() -> App {
    ...
    app.handle(Event::Changes {
        pending: vec![
            ...
            change(106, "other-box", ChangeStatus::Pending, false, "REVERT"),
        ],
...
fn tabs_partition_pending_changelists() {
    let app = app();
    // 395 and 308 are ours and unshelved; 166 is shelved; 106 is another box.
    assert_eq!(app.tab_count(ChangeTab::Local), 2);
...
fn the_tab_bar_shows_counts_and_marks_the_open_tab() {
    let out = render(&app(), 120, 40);
    assert!(out.contains("Local 2"), "{out}");
...
fn warns_when_no_client_is_set() {
    ...
    assert!(render(&app, 120, 40).contains("not set"));
    // With no client of our own, nothing counts as local.
    assert_eq!(app.tab_count(ChangeTab::Local), 0);
    assert_eq!(app.tab_count(ChangeTab::Others), 4);
}
