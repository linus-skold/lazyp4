...
impl Action {
    ...
    pub fn help(self) -> Option<Help> {
        ...
        match self {
            ...
            Action::Move => row("move to / from the change", None),
            Action::RevertFiles => row("revert (discard changes)", None),
            Action::ShelveFiles => row("shelve just these", None),
            Action::SelectRange => row("select a range", None),
            Action::History => row("revision history", None),
            Action::Blame => row("blame, line by line", None),
            Action::Ignore => row("ignore an untracked file", None),
            // `p4 status`: what was changed on disk without telling Perforce,
            // which is what `p4 reconcile` acts on.
            Action::Scan => row("find offline work (slow)", None),
